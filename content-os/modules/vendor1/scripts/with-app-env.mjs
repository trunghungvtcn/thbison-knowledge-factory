...
function main(argv) {
  ...
  const localWindowsCommand = join(projectRoot(), "node_modules", ".bin", `${command}.cmd`);
  const child = process.platform === "win32" && existsSync(localWindowsCommand)
    ? spawn(process.env.ComSpec || "cmd.exe", ["/d", "/s", "/c", localWindowsCommand, ...args], { stdio: "inherit", env })
